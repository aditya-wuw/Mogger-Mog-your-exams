...
export async function POST(req: Request) {
  ...
  {"question": "5) What is the name of the tower that appears during the Dark Hour in Persona 3 Reloaded?", "options": ["Tartarus", "Mementos", "TV World", "Abyss of Time"]}] and so on based on the users prompt in the quoats, strictly stick to only 4 options, make sure to only return the said format from the example, if the user dosen't send a specific prompt then simply generate 20 questions on general knowledge, if the user specified a topic but didn't mention the amount of questions then generate qustions in the range of 10 to 20 and make sure all the questions are unique based on the user's prompt, here is the usera's prompt : ${prompt}, also generate a answer key for the questions and now we must have two sperate arrays one the question key and another the answer. Now wrap the both the keys in an object for example { "questions_key" : the question array, "answer_key" : the array of only the answers which is a string for example ["answer1","answer2"] } and return the final out put`
