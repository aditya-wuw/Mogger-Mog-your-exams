...
const Page = () => {
  ...
  async function handleMogging() {
    ...
    if (prompt) {
      const prompt: boolean = confirm(
        "The questions will be saved and can be accesed through History page",
      );
      if (prompt) {
        setquestions([]);
    ...
    <div className="relative  mt-5">
      <main className="mx-3 lg:flex-row flex flex-col gap-2">
        <div className="lg:w-full md:h-120   p-2  select-none">
          <h1 className="md:text-5xl text-3xl mb-3 select-none ">Answer the following questions</h1>
          <section className="questions relative overflow-hidden warp-break-words h-[89%] rounded-md border border-green-900">
            ...
            <div className="md:absolute md:p-0 p-3 right-5 bottom-5 flex gap-2 justify-end">
              <button
                className="cursor-pointer rounded-xl bg-green-700 p-3 w-10 text-white hover:w-16 transtion-w duration-100 ease-in-out"
                onClick={() => {
              ...
              {count + 1 === TestObject.length ? (
                <button
                  ...
                  className={` bg-green-700 p-3 text-white rounded-xl hover:bg-green-400 ${Answer.length === 0 ? "cursor-not-allowed" : "cursor-pointer"}`}
                ...
                <div>
                  <button
                    className="cursor-pointer bg-green-700 p-3 w-10 text-white hover:w-16 duration-100 ease-in-out flex justify-end rounded-xl"
                    onClick={() => {
