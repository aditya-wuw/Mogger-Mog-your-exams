...
const Toast = ({ message, type_of }: Toaster) => {
  ...
  useEffect(()=>{
    ...
    const timeout = setTimeout(()=>{setToastMount(false)},6000)
    return () => {
  ...
  if(ToastMount) return (
    <div className={`absolute flex justify-center w-full lg:top-10 md:top-15 top-20`}>
      <motion.div
      ...
      >
        ...
        <span className="overflow-hidden truncate w-[85%]">{message}</span>
        <span>
          <button
